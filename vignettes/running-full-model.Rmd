...
* \color{red}`intervention`\color{black} These are kept fixed for all runs.
...
* \color{red}`policy`\color{black} These are modified but fixed for a set of `scenarios`.
  + `incidence_list`: which incidence by country of origin groups to target for LTBI screening. WHO categories per 100,000 are $[0,50),[50,150),[150,250),[250,350),>350$
  + `endpoint`: calculate QALYs and costs including those that exit EWNI or not i.e. time horizon
  + `LTBI_test`: type of test QFT, QFT-plus, TSPOT.TB
  + `treatment`: 6 months or 3 month LTBI treatment

* \color{red}`scenario`\color{black} These are varied within `policies`. We don't vary any health state utilities.
